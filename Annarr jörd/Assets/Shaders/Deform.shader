Shader "MyShaders/Deform"{
	Properties{
		...
		_DeformFactor("Deform Factor", Range(0,5)) = 1
	}
	...
	SubShader{
		Pass{
			...
			float4 _Color;
			sampler2D _MainTex, _NoiseTex, _DeformTex;
			float4 _MainTex_ST, _NoiseTex_ST, _DeformTex_ST;
			...
			{
				VetexToFragment v2f;
				float4 deformColor = tex2Dlod(_DeformTex, float4(vertex.uv * _DeformTex_ST.xy + _DeformTex_ST.zw, 0, 0));
				vertex.position.y = vertex.position.y + (deformColor.r * _DeformFactor);
